...
/**
 * Lightweight version of {@link pl.droidsonroids.gif.GifDrawable} used to
 * retrieve metadata of GIF only, without having to allocate the memory for its
 * pixels.
 */
public class GifAnimationMetaData implements Serializable, Parcelable {
	 ...
	 *             if res is null
	 */
	public GifAnimationMetaData(@NonNull Resources res,
			@DrawableRes @RawRes int id) throws Resources.NotFoundException,
			IOException {
	 ...
	 */
	public GifAnimationMetaData(@NonNull AssetManager assets,
	 ...
	 * be called from main thread. However it will violate
	...
	/**
	 * Retrieves metadata from InputStream. InputStream must support marking,
	...
	/**
	 * Retrieves metadata from AssetFileDescriptor. Convenience wrapper for
	 * {@link GifAnimationMetaData#GifAnimationMetaData(FileDescriptor)}
	 * 
	 * @param afd
	 *            source
	 * @throws NullPointerException
	 ...
	 *             when opening failed
	 */
	public GifAnimationMetaData(@NonNull AssetFileDescriptor afd)
			throws IOException {
	...
	/**
	 * Retrieves metadata from {@link ByteBuffer}. Only direct buffers are
	...
	/**
	 * Retrieves metadata from {@link android.net.Uri} which is resolved using
	 * {@code resolver}.
	 ...
	 */
	public GifAnimationMetaData(@Nullable ContentResolver resolver,
	...
	/**
	 * @return true if GIF is animated (has at least 2 frames and positive
	...
	public String toString() {
		String loopCount = mLoopCount == 0 ? "Infinity" : Integer
				.toString(mLoopCount);
