...
public class SlideView extends FrameLayout {
	...
	public static interface OnSwipeChangeListener {
		void onDraging(SlideView mSwipeLayout);

		void onOpen(SlideView mSwipeLayout);

		void onClose(SlideView mSwipeLayout);
		
		void onStartOpen(SlideView mSwipeLayout);
		
		void onStartClose(SlideView mSwipeLayout);
	...
	private ViewDragHelper.Callback callback = new ViewDragHelper.Callback() {
		...
		public int clampViewPositionHorizontal(View child, int left, int dx) {
			...
			if(!isSwipeEnable){
				//将不可以滑动判断处理移植此处 update 20170420 by lihai
				return 0;
			}
		...
		public void onViewReleased(View releasedChild, float xvel, float yvel) {
			if (xvel == 0 && frontView.getLeft() < -range * 0.3f) {
				open();
			} else if (xvel < 0) {
				open();
			} else {
...
//		bringChildToFront(frontView);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		/**
		 * 是否判断可滑动不可以放在这里，有时候点击事件分发不上去，导致有时候个别ITem点击无响应
		 * update 20170420 by lihai
		 */
		//if(isSwipeEnable)
