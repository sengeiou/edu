package com.ubt.alpha1e.edu.library.UbtBanner;
...
import com.ubt.alpha1e.edu.library.UbtBanner.adapter.CBPageAdapter;
import com.ubt.alpha1e.edu.library.UbtBanner.holder.CBViewHolderCreator;
import com.ubt.alpha1e.edu.library.UbtBanner.listener.CBPageChangeListener;
import com.ubt.alpha1e.edu.library.UbtBanner.listener.OnItemClickListener;
import com.ubt.alpha1e.edu.library.UbtBanner.view.CBLoopViewPager;
import com.ubt.alpha1e.edu.library.UbtBanner.view.ViewPagerScroller;
...
public class ConvenientBanner<T> extends LinearLayout {
        ...
        public void run() {
            ...
            if(convenientBanner != null){
                if (convenientBanner.viewPager != null && convenientBanner.turning) {
                    if(convenientBanner.viewPager.getAdapter() != null && convenientBanner.viewPager.getAdapter().getCount() > 0){
                        int page = convenientBanner.viewPager.getCurrentItem() + 1;
                        //UbtLog.e("convenientBanner","convenientBanner.page = " + page + "   count = " + convenientBanner.viewPager.getAdapter().getCount());
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_UP||action == MotionEvent.ACTION_CANCEL||action == MotionEvent.ACTION_OUTSIDE) {
            ...
            getParent().requestDisallowInterceptTouchEvent(true);//true:所有父view跳过事件分发，子view接管触摸事件
            lastX = x;
            lastY = y;
        }else if(action == MotionEvent.ACTION_MOVE)
        {
            int dealtX = x - lastX;
            int dealtY = y - lastY;
            if ((Math.abs(dealtX)-Math.abs(dealtY)>mTouchSlop)){
